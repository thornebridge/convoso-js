...
describe('SmsOptOut', () => {
  const client = createMockClient((path, params) => {
    if (path === '/sms-opt-out/search') return { success: true, offset: 0, limit: 100, total: 1, records: [{ id: '9001', phone_number: '5558887777', phone_code: '1', reason: 'User requested', purpose: 'Marketing', insert_date: '2026-03-01', campaign_id: '0', campaign_name: 'Global' }] };
  ...
  it('insert()', async () => {
    const result = await client.smsOptOut.insert({ phone_number: '5558887777', phone_code: '1', campaign_id: '0' });
